...
# members.all_match(chronotype_attr, wrt = comp, level = 1) # group_members.all_match(wrt: dim)
...
# time.match(0, 1, variable=subjects)
...
# members.match(chronotype_attr, wrt = comp, level = 0)
for i in range(2):
    members.match(i, i+1, variable = chronotype_attr, factor = 2, level = 0)

members.different(0, 1, variable = chronotype_attr, factor = 2, level = 1)

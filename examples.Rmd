...
# Define constructor
Task <- function(name) {
  # Vak()idate and create an instance of the class
  new("Task", name = name, types = list("all"))
}
...
setMethod(
  ...
  definition = function(object) {
    ...
    cat("types:", unlist(object@types), "\n")
  }
)

setGeneric("shares_type")
setMethod(
  f = "shares_type",
  signature = "Task",
  definition = function(object) {
...
share_type(block(1), block(3), types=(g1, g2))
...
# difference here is that types consists of individual tasks
apply(conditions, seg1)
match_order(seg1, seg2, types=conditions)
# -> {ffl, latex, ffl, latex}
#    {latex, ffl, latex, ffl}
# if seg2, then
...
Question: do we represent ffl and latex as separate from the tasks? 
for example, what if the user wants to order: ffl-c1, ffl-c2, latex-c1, latex-c2
could we use a function gen-conditions(treatment)
...
# params = (task you want to match, type description you
# want to match, set you are samplign from)
# output: e1 since e1 is the only element in the
# list that has an index label of "1"
